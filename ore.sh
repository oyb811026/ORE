...
function install_node() {
    ...
    brew update
    echo "安装必要的工具和依赖..."
    brew install curl git jq pkg-config
    ...
    # 将路径添加到 .bash_profile 或 .zshrc
    if [ -f ~/.bash_profile ]; then
        grep -qxF 'export PATH="$HOME/.local/share/solana/install/active_release/bin:$PATH"' ~/.bash_profile || echo 'export PATH="$HOME/.local/share/solana/install/active_release/bin:$PATH"' >> ~/.bash_profile
    fi
...
function export_wallet() {
    ...
    # 检查并将Solana的路径添加到 .bash_profile 或 .zshrc
    if [ -f ~/.bash_profile ]; then
        grep -qxF 'export PATH="$HOME/.local/share/solana/install/active_release/bin:$PATH"' ~/.bash_profile || echo 'export PATH="$HOME/.local/share/solana/install/active_release/bin:$PATH"' >> ~/.bash_profile
    fi
...
function main_menu() {
    while true; do
        ...
        echo "11. 单独更换rpc等配置，并多开自动读取/.config/solana 下所有json私钥文件并且私钥前缀命名监控，请提前安装好jq"
        echo "12.算力测试"
