...
function jito() {
    ...
    # 获取用户输入的核心数量或使用默认值
    read -p "请输入要使用的核心数量 (默认设置为 1): " cores
    ...
    start="while true; do ore --rpc $RPC_URL --keypair ~/id.json --priority-fee $PRIORITY_FEE mine --cores $threads; echo '进程异常退出，等待重启' >&2; sleep 1; done"
    screen -dmS "$session_name" bash -c "$start"
